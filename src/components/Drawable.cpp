...
void Drawable::fcircle(float x, float y, float radius, const Color color = Color::Cyan, const int triangles = 20) {
    ...
    for (int i = 0; i <= triangles; i++) {
        glVertex2f(x + (radius * cos(i * twicePi / triangles)), y + (radius * sin(i * twicePi / triangles)));
    }
    glEnd();
}
...
void Drawable::ChangeColor(const Color _color) { this->color = _color; }

Vector2D Drawable::getScreenSize() { return physics::Vector2D(glutGet(GLUT_WINDOW_WIDTH), glutGet(GLUT_WINDOW_HEIGHT)); }

void Drawable::Setup(void) { transform = gameObject->GetComponent<Transform>(); }

void Drawable::Update(void) { Draw(); }
...
void Circle::Draw(void) { fcircle(transform->position.x, transform->position.y, raidius, color, detail); }
...
void Square::Draw(void) {
    ...
    rect(transform->position.x - halfDiameter + offset.x, transform->position.y - halfDiameter + offset.y, transform->position.x + halfDiameter + offset.x,
         transform->position.y + halfDiameter + offset.y);
}
void Square::Center(const Vector2D _offset = Vector2D(0, 0)) { offset = _offset; }
...
// }
void Line::Draw(void) { line(startingOffset.x, startingOffset.y, endingOffset.x, endingOffset.y, color); }
...
void ForceLines::Setup(void) { Drawable::Setup(); }
